...
const Welcome = () => {
  const { tiles, fetchTiles, tileCategories, fetchTileCategories } = useAuth();

  useEffect(() => {
    fetchTiles();
    fetchTileCategories();
  }, [fetchTiles, fetchTileCategories]);
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <p className="text-2xl text-[#bd5b4c]">{tiles.length}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-semibold mb-2">Categories</h3>
            <p className="text-2xl text-[#bd5b4c]">{tileCategories.length}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
